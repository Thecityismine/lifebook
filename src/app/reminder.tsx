...
export default function ReminderScreen() {
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <Pressable disabled={updating || Boolean(reminder.archivedAt)} onPress={() => void changeCompleted()} style={[styles.completeButton, reminder.completedAt && styles.reopenButton, reminder.archivedAt && styles.disabled]}>{updating ? <ActivityIndicator color={AppColors.onAccent} /> : <><Ionicons name={reminder.completedAt ? 'refresh' : 'checkmark-circle'} size={21} color={AppColors.onAccent} /><Text style={styles.completeButtonText}>{reminder.completedAt ? 'Mark as upcoming' : 'Mark complete'}</Text></>}</Pressable>
...
const styles = StyleSheet.create({
  ...
  completeButton: { minHeight: 56, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: Spacing.sm, borderRadius: Radius.lg, backgroundColor: AppColors.mint }, reopenButton: { backgroundColor: AppColors.violet }, completeButtonText: { color: AppColors.onAccent, fontSize: 15, fontWeight: '800' }, disabled: { opacity: 0.42 }, card: { padding: Spacing.xl, gap: Spacing.md, borderRadius: Radius.lg, backgroundColor: AppColors.paper, borderWidth: 1, borderColor: AppColors.border, ...Shadow.card }, sectionTitle: { color: AppColors.ink, fontSize: 18, fontWeight: '700', textAlign: 'left' }, notes: { color: AppColors.ink, fontSize: 15, lineHeight: 24 }, emptyValue: { color: AppColors.slate, fontSize: 14, lineHeight: 21, fontStyle: 'italic' },
